...
async def apiSession(request: Request):
    """サーバー発行のセッショントークンを新規作成 (secrets使用)。"""
    ip = clientIp(request)
...
async def apiProfile(body: ProfileBody, request: Request):
    ...
    result = await canvas.doProfile(token, body.name, body.color)
...
async def apiPlace(body: PlaceInput, request: Request):
    result = await canvas.doPlace(body, ip=clientIp(request))
    if result.get("ok"):
        await sio.emit(
            "pixel",
            {
...
async def apiUndo(body: UndoBody):
    result = await canvas.undoPlace(body)
    if result.get("ok"):
        await sio.emit(
            "pixel",
            {
...
async def apiAccountIssue(body: AccountIssueBody, request: Request):
    ...
    db = await getDb()
    async with dbLock:
        await users.ensureUser(db, token)
...
async def apiAccountLogin(body: AccountLoginBody, request: Request):
    ...
    async with db.execute(
        ...
        " transferCode, passwordHash FROM users WHERE transferCode = ?",
        (code,),
    ) as cur:
