...
- 仕組み: アカウントパネルの「旧サイトから引き継ぐ」はトップフレームで
  旧 `/migrate?to=...&handoff=1` を開く (ファーストパーティのため分離の影響なし)。
  shimは読み取ったデータをURL断片 (`#mig=`) に載せて新サイトに戻し
  (断片はサーバーに送られない)、新サイトは起動時に消費して保存・reloadする
  (`static/js/wd-migrate.js`)。無効トークンは `/api/me` で検証して除外し、設定のみ残す。
- 断片の安全枠 (~8KB) を超える場合は設計図等の bulky なキーを除外して
  部分引っ越しする (落とした項目名をトースト表示。元データは旧側に残る)。
- 旧ドメインへの直接訪問 (旧ブックマーク等) も、運ぶデータがあれば自動で
  ハンドオフして新サイトへ送る (空なら素通し転送)。新側に使用中の
  アカウントがある場合、自動分は上書きしない (手動は確認済みのため上書きする)。
- 旧 `/` はエッジで新 `/` へ転送されるため、直打ち到達時は新側が Referer を
  見て一度だけ自動往復する (未使用時のみ。使用中・試行済みは不干渉)。
  これにより直打ちでもアカウントが自動移行する。
- 初訪時の自動引っ越しは隠しiframeのまま (ベストエフォート。
  サードパーティ分離のあるブラウザでは黙って失敗する)。
